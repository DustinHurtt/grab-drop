...
sheep.src =
  "sheep_on_a_transparent_background_by_zoostock_ddhetu0-fullview.png";

let spaceshipblueprint = {
  x: w / 2,
  y: h / 5,
  w: 40,
  h: 40,
...
let Cd = 0.47;
let rho = 1.22;
let A = (Math.PI * spaceship.radius * spaceship.radius) / 10000;
let ag = 1.2;
let friction = 0.01;
...
function updatePosition(obj) {
  ...
  let Fx = (-0.5 * Cd * A * rho * obj.vx * obj.vx * obj.vx) / Math.abs(obj.vx);
  let Fy = (-0.5 * Cd * A * rho * obj.vy * obj.vy * obj.vy) / Math.abs(obj.vy);

  Fx = isNaN(Fx) ? 0 : Fx;
  Fy = isNaN(Fy) ? 0 : Fy;

  let ax = Fx / obj.mass;
  let ay = ag + Fy / obj.mass;
...
function dropObject() {
  if (spaceship.grabbedItems.length > 0) {
    ...
    if (spaceship.grabbedItems.length < 1) {
      ...
      spaceship.fillColor =
        colors[spaceship.grabbedItems[spaceship.grabbedItems.length - 1].color];
    }
...
function gameOver() {
  ...
  if (droppedArr.length > 0) {
    ...
    ctx.fillText(
      `${droppedArr.length} farm animals failed to beam down, and are subtracted from your score.`,
      w / 2,
      250
    );
  }
  ...
  ctx.fillText(
    `Final Score: ${spaceship.score - droppedArr.length}`,
    w / 2,
    400
  );
  if (spaceship.score - droppedArr.length < 10) {
    ...
    ctx.fillText(
      `You have failed and will be sent to the Delta Quadrant!`,
      w / 2,
      500
    );
  }
...
function gameLoop(currentTime) {
  ...
  for (let i = 0; i < droppedArr.length; i++) {
    ...
    updatePosition(droppedObj, deltaTime);
  ...
  updatePosition(spaceship, deltaTime);
...
function startGame() {
  if (gameOn === false) {
    ...
    game = setInterval(gameLoop, 1000 / 60, performance.now());
  }
}
